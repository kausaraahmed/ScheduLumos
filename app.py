...
def round_robin():
    if request.method == 'POST':
        arrival_times = request.form.get('arrival-time')
        burst_times = request.form.get('burst-time')
        time_quantam = request.form.get('time-quantam')

        return run_rr(arrival_times, burst_times, time_quantam)

    return render_template('round_robin.html')


def run_rr(arrival_times, burst_times, time_quantam):
        try:
            arrival_times = list(map(int, arrival_times.strip().split(' ')))
            burst_times = list(map(int, burst_times.strip().split(' ')))
    ...
    if request.method == 'POST':
        ...
        if algo == None:
            ...
            if algo == 'sjfp':
                ...
                return run_rr(global_arrival_times, global_burst_times, global_time_quantam)
